...
import EnvironmentKnowledge from "./intro/EnvironmentKnowledge";
import MFQ from "./intro/MFQ";
import MFQ2 from "./intro/MFQ2";
...
Empirica.introSteps((_game, treatment) => {
  const steps = [InstructionStepOne];
  if (treatment.playerCount > 1) {  //for this, if number of players is more than 1, then we can do a different thing. For GTC, it was number of alters
    steps.push(InstructionStepTwo);
  }
  steps.push(overview);
  steps.push(Quiz);
  steps.push(EnvironmentKnowledge);
  //steps.push(MFQ);
  //steps.push(MFQ2);
  return steps;
});
